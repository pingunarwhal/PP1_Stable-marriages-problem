...
(define (get-women-helper wpref acc)
  (if (equal? wpref '())
      acc
      (get-women-helper (cdr wpref) (append acc (list (car (car wpref)))))))
...
(define (get-pref-list pref person)
  (cond
    [(equal? pref '()) '()]
    [(equal? person (car (car pref))) (cdr (car pref))]
...
(define (preferable? pref-list x y)
  (or (equal? pref-list '()) (and (not (equal? (car pref-list) y)) (or (equal? (car pref-list) x) (preferable? (cdr pref-list) x y)))))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (if [equal? (car p1-list) p2]
      #f
      (or (better-match-exists? p1 p2 (cdr p1-list) pref2 engagements)
