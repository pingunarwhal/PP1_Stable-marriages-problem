...
(define (get-unstable-couples engagements mpref wpref)
  (let ([not-ideal? (lambda (engagement)
                         (let ([first (car engagement)]
                               [second (cdr engagement)])
                           (or
                            (better-match-exists? first second (get-pref-list wpref first) mpref engagements)
                            (better-match-exists? second first (get-pref-list mpref second) wpref (map (lambda (pair) (cons (cdr pair) (car pair))) engagements)))))])
