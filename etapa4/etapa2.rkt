...
(define (get-women wpref)
  (foldr (lambda (pref list) (cons (car pref) list)) '() wpref))
...
(define (get-pref-list pref person)
  (cdr (car (filter (lambda (list) (equal? (car list) person)) pref))))
...
(define (get-partner engagements person)
  (define x (find-first (lambda (pair) (equal? (car pair) person)) engagements))
    (if (equal? x #f)
        (let ([y (find-first (lambda (pair) (equal? (cdr pair) person)) engagements)])
          (if (equal? y #f)
              #f
              (car y)))
        (cdr x)))
...
; varianta cu let:
;(define (get-partner engagements person)
;  (let ([x (find-first (lambda (pair) (equal? (car pair) person)) engagements)])
;    (if (equal? x #f)
;        #f
;        (cdr x))))
...
(define (update-engagements engagements p1 p2)
  (change-first (lambda (pair) (equal? (car pair) p1)) engagements (cons p1 p2)))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (if [equal? (car p1-list) p2]
      #f
      (if (preferable? (get-pref-list pref2 (car p1-list)) p1 (get-partner (map (lambda (pair) (cons (cdr pair) (car pair))) engagements) (car p1-list)))
          #t
...
(define (stable-match? engagements mpref wpref)
  (if (null? engagements)
      #t
      (if (better-match-exists? (car (car engagements)) (cdr (car engagements)) (get-pref-list wpref (car (car engagements))) mpref engagements)
          #f
          (if (better-match-exists? (cdr (car engagements)) (car (car engagements)) (get-pref-list mpref (cdr (car engagements))) wpref engagements)
              #f
